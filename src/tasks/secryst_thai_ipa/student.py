"""Thai→IPA student model.

Architecture shared with rababa's ``_CharTransformer`` (DRY). Only the
vocab sizes differ. Both tasks demonstrate that the framework is
script-agnostic — add a new transliteration pair = new data + config,
...
@register_model_module("secryst_student")
class SecrystThaiIpaStudent(ModelModule):
    """Thai→IPA variant of the char-level transformer."""

    kind = ModelKind.STUDENT

    def __init__(self, config: ModelConfig) -> None:
        from tasks.secryst_thai_ipa.data import INPUT_VOCAB, OUTPUT_VOCAB
    ...
    def generate(self, input_ids, attention_mask=None, max_new_tokens=128) -> GenerateOutput:
        ...
        from tasks.secryst_thai_ipa.data import OUTPUT_VOCAB
