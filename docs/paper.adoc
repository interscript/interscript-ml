...
Memory closes 0.70pp of the 5.68pp teacher–student gap — a real, controlled improvement at near-zero added compute, but below the pre-registered 1.0pp bar under which we would have called the capacity axis *the* fix. The honest conclusion: the distillation gap at this rung is dominated not by parameter capacity but by optimization and domain coverage (the student's training labels span one corpus's domain; the full-benchmark subset lesson of <<section-leaderboard>> measures exactly this exposure). Capacity is a lever on this frontier — a modest one, now measured.
